/*******************************************************************************
**NOTE** This code was generated by a tool and will occasionally be
overwritten. We welcome comments and issues regarding this code; they will be
addressed in the generation tool. If you wish to submit pull requests, please
do so for the templates in that tool.

This code was generated by Vipr (https://github.com/microsoft/vipr) using
the T4TemplateWriter (https://github.com/msopentech/vipr-t4templatewriter).

Copyright (c) Microsoft Corporation. All Rights Reserved.
Licensed under the Apache License 2.0; see LICENSE in the source repository
root for authoritative license information.﻿
******************************************************************************/



#ifndef MSGRAPHSERVICEENTITYFETCHER_H
#define MSGRAPHSERVICEENTITYFETCHER_H

#import "MSGraphServiceModels.h"
#import "api/api.h"
#import "core/core.h"
#import "core/MSOrcEntityFetcher.h"

@class MSGraphServiceDirectoryObjectFetcher;	
@class MSGraphServiceConversationThreadFetcher;	
@class MSGraphServiceCalendarFetcher;	
@class MSGraphServiceOutlookItemFetcher;	
@class MSGraphServiceConversationFetcher;	
@class MSGraphServiceProfilePhotoFetcher;	
@class MSGraphServiceDriveFetcher;	
@class MSGraphServiceSubscribedSkuFetcher;	
@class MSGraphServiceMailFolderFetcher;	
@class MSGraphServiceCalendarGroupFetcher;	
@class MSGraphServiceContactFolderFetcher;	
@class MSGraphServiceAttachmentFetcher;	
@class MSGraphServiceDriveItemFetcher;	
@class MSGraphServicePermissionFetcher;	
@class MSGraphServiceThumbnailSetFetcher;	
@class MSGraphServiceEntityOperations;


/** MSGraphServiceEntityFetcher
 *
 */
__deprecated_msg("This SDK is deprecated. Please review the README for further information (https://github.com/OfficeDev/Microsoft-Graph-SDK-iOS).")
@interface MSGraphServiceEntityFetcher : MSOrcEntityFetcher


@property (copy, nonatomic, readonly) MSGraphServiceEntityOperations *operations;

- (instancetype)initWithUrl:(NSString*)urlComponent parent:(id<MSOrcExecutable>)parent;
- (void)readWithCallback:(void (^)(MSGraphServiceEntity *, MSOrcError *))callback;
- (void)update:(MSGraphServiceEntity *)entity callback:(void (^)(MSGraphServiceEntity *, MSOrcError*))callback ;
- (void)delete:(void(^)(int status, MSOrcError *))callback;
- (MSGraphServiceEntityFetcher *)addCustomParametersWithName:(NSString *)name value:(id)value;
- (MSGraphServiceEntityFetcher *)addCustomHeaderWithName:(NSString *)name value:(NSString *)value;
- (MSGraphServiceEntityFetcher *)select:(NSString *)params;
- (MSGraphServiceEntityFetcher *)expand:(NSString *)value;
- (MSGraphServiceDirectoryObjectFetcher *)asDirectoryObject;	
- (MSGraphServiceConversationThreadFetcher *)asConversationThread;	
- (MSGraphServiceCalendarFetcher *)asCalendar;	
- (MSGraphServiceOutlookItemFetcher *)asOutlookItem;	
- (MSGraphServiceConversationFetcher *)asConversation;	
- (MSGraphServiceProfilePhotoFetcher *)asProfilePhoto;	
- (MSGraphServiceDriveFetcher *)asDrive;	
- (MSGraphServiceSubscribedSkuFetcher *)asSubscribedSku;	
- (MSGraphServiceMailFolderFetcher *)asMailFolder;	
- (MSGraphServiceCalendarGroupFetcher *)asCalendarGroup;	
- (MSGraphServiceContactFolderFetcher *)asContactFolder;	
- (MSGraphServiceAttachmentFetcher *)asAttachment;	
- (MSGraphServiceDriveItemFetcher *)asDriveItem;	
- (MSGraphServicePermissionFetcher *)asPermission;	
- (MSGraphServiceThumbnailSetFetcher *)asThumbnailSet;	

@end

#endif
