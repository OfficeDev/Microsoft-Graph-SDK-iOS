/*******************************************************************************
**NOTE** This code was generated by a tool and will occasionally be
overwritten. We welcome comments and issues regarding this code; they will be
addressed in the generation tool. If you wish to submit pull requests, please
do so for the templates in that tool.

This code was generated by Vipr (https://github.com/microsoft/vipr) using
the T4TemplateWriter (https://github.com/msopentech/vipr-t4templatewriter).

Copyright (c) Microsoft Corporation. All Rights Reserved.
Licensed under the Apache License 2.0; see LICENSE in the source repository
root for authoritative license information.﻿
******************************************************************************/


#ifndef MSGRAPHSERVICEATTACHMENT_H
#define MSGRAPHSERVICEATTACHMENT_H

#import <Foundation/Foundation.h>
#import "core/MSOrcChangesTrackingArray.h"

#import "MSGraphServiceEntity.h"
#import "api/MSOrcInteroperableWithDictionary.h"

/** Interface MSGraphServiceAttachment
 *
 */
__deprecated_msg("This SDK is deprecated. Please review the README for further information (https://github.com/OfficeDev/Microsoft-Graph-SDK-iOS).")
@interface MSGraphServiceAttachment : MSGraphServiceEntity <MSOrcInteroperableWithDictionary>


/** Property lastModifiedDateTime
 *
 */
@property (nonatomic,  copy, setter=setLastModifiedDateTime:, getter=lastModifiedDateTime) NSDate * lastModifiedDateTime;

/** Property name
 *
 */
@property (nonatomic,  copy, setter=setName:, getter=name) NSString * name;

/** Property contentType
 *
 */
@property (nonatomic,  copy, setter=setContentType:, getter=contentType) NSString * contentType;

/** Property size
 *
 */
@property (nonatomic,  setter=setSize:, getter=size) int size;

/** Property isInline
 *
 */
@property (nonatomic,  setter=setIsInline:, getter=isInline) bool isInline;


+ (NSDictionary *) $$$_$$$propertiesNamesMappings;


@end

#endif
